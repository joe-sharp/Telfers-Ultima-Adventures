    ...
    public class BaseHarvester : Container, IOneTime
    {
        ...
        public override void AddNameProperties(ObjectPropertyList list)
        {
            ...
            if (IsLiquidBarrel(this) && m_owner == null)
                list.Add("Can be placed anywhere, in or outside of a house.");
            else if (!IsLiquidBarrel(this) && this.RootParentEntity != null)
            {
                switch (m_type)
                {
                    case 1: list.Add("Must be placed on cave tiles, outside a house to function."); break;
                    case 2: list.Add("Must be placed on forest or jungle tiles, outside a house to function."); break;
                    case 3: list.Add("Must be placed on swamp tiles, outside a house to function."); break;
                    default: list.Add("Must be placed on the ground, outside a house to function."); break;
                }
            }
            ...
            if (Utility.RandomDouble() < ((double)this.m_quality / 10))
            {
                if (m_type <= 3)
                {
                    ...
                    if (this.m_type == 1) // mining
                    {
                        ...
                        if (m_owner.Skills[SkillName.Mining].Base < 100 && chance > 9)
                            chance = 9;

                        m_owner.CheckSkill(SkillName.Mining, 0, (m_quality * 10));

                        if (Utility.RandomDouble() < (1 - ((double)m_quality / 10)))
                        { resource = new IronOre(); resource.Amount = Utility.RandomMinMax(1, 5); }
                        else
                        {
                    ...
                    else if (this.m_type == 2) // lumber
                    {
                        ...
                        if (m_owner.Skills[SkillName.Lumberjacking].Base < 100 && chance > 7)
                            chance = 7;
                    ...
                    else if (this.m_type == 3) // leather
                    {
                        ...
                        if (m_owner.Skills[SkillName.Forensics].Base < 100 && chance > 8)
                            chance = 8;
                    ...
                    if (resource != null)
                    {
                        ...
                        if (Utility.RandomMinMax(1, (100 * m_quality)) == 69)
                        {
                            ...
                            ((ToxicSlug)slug).Owner = this.Owner;
                        }
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            // Staff inspecting the Harvester should not activate it.
            if (from.AccessLevel > AccessLevel.Player)
            {
                if (m_type < 10) // If it's a resource harvester, open it
                    Open(from);
                return;
            }
            ...
            if (this.m_owner == null) //check placement for 1. placed on ground, 2. proximity to another device and 3. proper ground tiles
            {
                ...
                if (!IsLiquidBarrel(this)) //barrels don't have area control limits
                {
                    ...
                    foreach (Item item in eable)
                    {
                        ...
                        else if (item is BaseHouse || item is HouseSign || item is TownHouseSign)
                        {
                            from.SendMessage(0, "This is too close to a home to operate.");
                            return;
                        }
                    }
                ...
                try
                {
                    ...
                    if (o != null && o is Mobile)
                    {
                        ...
                        if (c is BaseCreature)
                        {
                            ...
                            if (m is BasicSentry)
                            {
                                ((BasicSentry)m).Owner = this.Owner;
                            }
                            else if (m is SolidSentry)
                            {
                                ((SolidSentry)m).Owner = this.Owner;
                            }
                            else if (m is StrongSentry)
                            {
                                ((StrongSentry)m).Owner = this.Owner;
                            }
                            else if (m is WeakSentry)
                            {
                                ((WeakSentry)m).Owner = this.Owner;
                            }
