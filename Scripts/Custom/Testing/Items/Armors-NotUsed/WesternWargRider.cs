...
public class WesternWargRider : BaseCreature
...
case 0: return WeaponAbility.WhirlwindAttack; case 1: return WeaponAbility.ConcussionBlow; }}
private DateTime m_NextFastHeal;[Constructable] 
public WesternWargRider() : base( AIType.AI_Melee, FightMode.Closest, 10, 1, 0.2, 0.4 ) 
...
Karma = -5000;SetStr( 386, 400 );SetDex( 151, 165 );SetInt( 161, 175 );SetDamage( 18, 19 );
...
AddItem( new StuddedChest());AddItem( new StuddedLegs());AddItem( new Halberd());
AddItem( new ThighBoots());WesternWarg mt = new WesternWarg();mt.Rider = this;}
public override void GenerateLoot(){AddLoot( LootPack.FilthyRich );}
public override void OnThink(){if (this.m_NextFastHeal < DateTime.UtcNow){
if (this.Hits < this.HitsMax){this.Hits += (this.HitsMax / 6);
this.m_NextFastHeal = DateTime.UtcNow + TimeSpan.FromSeconds(10);}}}
public override bool OnBeforeDeath(){IMount mount = this.Mount;if ( mount != null ){
mount.Rider = null;}return base.OnBeforeDeath();}public WesternWargRider( Serial serial ) : 
base( serial ){}public override void Serialize( GenericWriter writer ){base.Serialize( writer ); 
