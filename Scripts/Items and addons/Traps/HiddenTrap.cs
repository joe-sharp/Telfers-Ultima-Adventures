...
namespace Server.Items
{
	public class HiddenTrap : Item
	{
        ...
        public static Item GetMyItem( Mobile m )
		{
            ...
            int startingOffset = Utility.RandomMinMax(1, _layers.Length);
            for (int i = 0; i < _layers.Length; i++)
            {
                var layerIndex = (startingOffset + i) % _layers.Length;
                Item item = TryGetDestroyableItem(m, _layers[layerIndex]);
